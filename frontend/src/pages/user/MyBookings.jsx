...
const MyBookings = () => {
  ...
  const getAllBookings = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `http://localhost:8000/api/booking/get-UserCurrentBookings/${currentUser?._id}?searchTerm=${searchTerm}`
      , {
        method: "GET",
        headers: {
  ...
  const handleCancel = async (id) => {
    try {
      setLoading(true);
      const res = await fetch(
        `http://localhost:8000/api/booking/cancel-booking/${id}/${currentUser._id}`,
        {
          method: "POST",
        }
      );
